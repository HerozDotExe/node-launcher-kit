...
test("parse arguments correctly for 1.21.8", { timeout: 0 }, async () => {
  const versionJar = path.join(root, "1.21.8.jar")
  const args = await nlk.core.arguments.generateLaunchArguments(
    await nlk.core.version.getVersionManifest("1.21.8", root),
    path.join(root, "java"),
    path.join(root),
    versionJar,
    offline(),
